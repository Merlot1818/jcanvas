...
(function (jQuery, global, factory) {
	...
	function cloud(ctx, rect, r) {
		var start = {x: rect.x, y: rect.y};
		var end = {x: rect.x + rect.width, y: rect.y + rect.height};
		ctx.beginPath();
		ctx.strokeStyle = 'black';
		ctx.lineWidth = 2;
		ctx.moveTo(start.x, start.y);
		ctx.fillStyle = 'red';
		for (var i = start.x + r; i < end.x - r; i += 2 * r) {
			ctx.arc(i, start.y, r, Number(Math.PI), 2 * Math.PI);
		}

		for (var i = start.y + r; i < end.y - r; i += 2 * r) {
			ctx.arc(end.x - r * 2, i, r, 3 / 2 * Math.PI, 1 / 2 * Math.PI);
		}

		for (var i = end.x - r; i > start.x + r; i -= 2 * r) {
			ctx.arc(i - r * 2, end.y - r, r, 0 * Math.PI, Number(Math.PI));
		}
		for (var i = end.y - r; i > start.y + r; i -= 2 * r) {
			ctx.arc(start.x, i - r, r, 1 / 2 * Math.PI, 3 / 2 * Math.PI);
		}
	...
	// Draws ellipse
	$.fn.drawCloud = function drawCloud(args) {
		var $canvases = this, e, ctx,
			params,
			controlW,
			controlH;
		...
		for (e = 0; e < $canvases.length; e += 1) {
			...
			if (ctx) {
				...
				params = new jCanvasObject(args);
				_addLayer($canvases[e], params, args, drawCloud);
				if (params.visible) {
					...
					// ctx.lineWidth = 2;
					// ctx.strokeStyle = 'red';
					// ctx.moveTo(params.x - controlW / 2, params.y - controlH / 2);
					// var w = controlW / 60;
					// for (var i = params.x - controlW / 2; i < params.x + controlW / 2; i += 0.1) {
					// 	var x = i;
					// 	var y = params.y - controlH / 2 + Math.sin(i / w) * 6;
					// 	ctx.lineTo(x, y);
					// }
					// ctx.stroke();
					// ctx.closePath();
					...
					cloud(ctx, {x: params.x - controlW / 2, y: params.y - controlH / 2, width: controlW, height: controlH}, 6);
